...
namespace Meridian::Converters
{
                                                        ...
                                                        CefString& a_exception)
    {
        ...
        else if (a_v8Value->IsUInt())
        {
            result->SetDouble(a_v8Value->GetUIntValue());
        }
        ...
        else if (a_v8Value->IsArrayBuffer())
        {
    ...
    CefRefPtr<CefV8Value> CEFValueConverter::ConvertToV8(const CefRefPtr<CefValue>& a_value)
    {
        if (a_value == nullptr)
        {
        ...
        {
        case VTYPE_BOOL:
            return CefV8Value::CreateBool(a_value->GetBool());
        case VTYPE_INT:
            return CefV8Value::CreateInt(a_value->GetInt());
        case VTYPE_DOUBLE:
            return CefV8Value::CreateDouble(a_value->GetDouble());
        case VTYPE_STRING:
            return CefV8Value::CreateString(a_value->GetString());
        case VTYPE_LIST: {
            const auto list = a_value->GetList();
            const auto length = list == nullptr ? 0 : static_cast<int>(list->GetSize());
            auto arr = CefV8Value::CreateArray(length);
            for (int i = 0; i < length; ++i)
            {
                arr->SetValue(i, ConvertToV8(list->GetValue(i)));
            }
            return arr;
        }
        case VTYPE_DICTIONARY: {
            auto obj = CefV8Value::CreateObject(nullptr, nullptr);
            const auto dict = a_value->GetDictionary();
            if (dict != nullptr)
            {
                CefDictionaryValue::KeyList keys;
                dict->GetKeys(keys);
                for (const auto& key : keys)
                {
                    obj->SetValue(key, ConvertToV8(dict->GetValue(key)), V8_PROPERTY_ATTRIBUTE_NONE);
                }
            }
            return obj;
        }
